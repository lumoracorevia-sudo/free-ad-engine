import streamlit as st; import pandas as pd; import time
...
if submit_button:
    if campaign_name and ad_content and destination_url:
        with st.spinner("Injecting ads into targeting algorithm..."):
            time.sleep(2); tracking_id = f"REF_{int(time.time())}"; tracking_link = f"{destination_url}?ref={tracking_id}"
            ...
            new_data = {"ID": tracking_id, "Name": campaign_name, "Platform": target_platform, "Tracking Link": tracking_link, "Media": media_status, "Views": 0, "Clicks": 0, "Status": "Active"}
            st.session_state.campaigns.append(new_data); st.success(f"✅ Campaign Launched! Media Processed: {media_status}")
...
if st.session_state.campaigns:
    ...
    st.dataframe(df[["Name", "Platform", "Media", "Views", "Clicks", "Status"]])
